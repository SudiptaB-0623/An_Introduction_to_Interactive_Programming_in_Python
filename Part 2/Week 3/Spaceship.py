...
def rock_spawner ():
    global a_rock
    a_rock = Sprite ([random.randrange (w), random.randrange (h)], [random.choice ([0.5, -1]) * random.random () * 2, random.choice ([0.5, -1]) * random.random () * 2], 1, random.choice ([0.5, -1]) * 0.05, asteroid_image, asteroid_info)


# initialize frame
frame = simplegui.create_frame ("Asteroids", w, h)

# initialize ship and two sprites
my_ship = Ship ([w / 2, h / 2], [0, 0], 0, ship_image, ship_info)
a_rock = Sprite ([w / 3, h / 3], [1, 1], 0, 0, asteroid_image, asteroid_info)
